...
visual_frame.grid(row=1, column=0, pady=5)
for _ in range(7):
    lbl = tk.Label(visual_frame, text='', width=3, height=1, font=('Arial', 20), relief='groove')
    lbl.pack(side=tk.LEFT, padx=3)
    visual_labels.append(lbl)

tk.Label(main_frame, text="초성/중성/종성을 선택 후 자모를 고르세요", font=("Arial", 12)).grid(row=2, column=0, pady=10)

active_label = tk.Label(main_frame, text="선택 중: 종성", font=("Arial", 14))
active_label.grid(row=3, column=0, pady=5)
...
choice_frame.grid(row=5, column=0, pady=20)
